...
const AstrologerProfile = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.post(`${import.meta.env.VITE_ASTRO_URL}/upload-documents`, formData, {
        headers: {
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="flex-1 p-6">
        <h1 className="text-2xl font-semibold mb-6 text-gray-800">Upload Documents</h1>

        <div className="bg-white p-6 rounded-2xl shadow-md space-y-5 max-w-xl">
          ...
          <button
            ...
            disabled={loading}
            className="flex items-center gap-2 px-5 py-2 mt-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
              <svg
